    ...
    for attempt in range(100):
        try: 
            if judge_model == "openai/qwen2.5-72b-instruct":
                ...
                json_schema_string = json.dumps(extracted_answer_format_for_confidence['json_schema'], indent=2)
                deepseek_prompt = f"""{prompt}

Your response MUST be a single JSON object matching the following schema. Do not include any other text, explanations, or markdown formatting like ```json.

Schema:
{json_schema_string}
"""
                client = OpenAI(api_key="sk-c567a01b77f54bb280deff93c4773703", base_url="https://api.deepseek.com")
                ...
                try:
                    ...
                    if json_start != -1 and json_end != -1:
                        ...
                        raw_judge = json.loads(json_string)
                    else:
                        raise ValueError("No valid JSON object found in the model's response.")
                except (json.JSONDecodeError, ValueError) as e:
                    print(f"Failed to parse JSON from DeepSeek response. Content: {raw_content}. Error: {e}")
                    raise e
...
def single_round_statistics(input_file):
    ...
    all_total_token_consumptions = [] # NEW: For token consumption metric
    termination_counts = {}
    ...
    for item in contents:
        # FIX: Use .get() for safety to prevent KeyError
        messages = item.get("messages", [])
        ...
        # NEW: Collect total token consumption
        all_total_token_consumptions.append(item.get("total_token_consumption", 0))

        termination = get_termination_value(item)
        termination_counts[termination] = termination_counts.get(termination, 0) + 1

        try:
            if messages and len(tokenizer.encode("".join([msg["content"] for msg in messages]))) > 30000:
                num_extra += 1  
        except:
            pass
    ...
    return {
        ...
        # NEW: Add average token consumption to stats
        "avg_total_token_consumption": sum(all_total_token_consumptions) / len(all_total_token_consumptions) if all_total_token_consumptions else 0,
...
def calculate_enhanced_statistics(round_results, round_items):
    ...
    correct_total_tokens = [] # NEW: For token consumption on correct answers

    for round_name in ["round1", "round2", "round3"]:
        results = round_results[round_name]
        items = round_items[round_name]

        question_to_item_map = {item.get("question", ""): item for item in items}
        ...
        for result in results:
            ...
            matching_item = question_to_item_map.get(question)
            
            if not matching_item or not matching_item.get("messages"):
                continue
            ...
            for msg in messages:
                if msg['role'] == 'assistant':
                    ...
                    if "<tool_call>" in content and "</tool_call>" in content:
                         num_tool_use += content.count("<tool_call>")
            ...
            # NEW: Collect total token consumption for correct answers
            correct_total_tokens.append(item.get("total_token_consumption", 0))
    ...
    return {
        ...
        "avg_total_token_consumption_correctly_solved": round(avg_total_tokens_correct, 3) # NEW
    }
...
def calculate_pass_at_k(query_results, k=10): 
    ...
    overall_pass = total_correct / len(query_results) if query_results else 0
...
def calculate_best_pass_at_1(query_results):  
    ...
    if not query_results: return 0.0
...
def main():
    ...
    if not os.path.isdir(input_path):
        print(f"Warning: --input_folder '{input_path}' is not a directory. Assuming it's a file and using its parent directory.")
        input_path = os.path.dirname(input_path)
    
    round1_file = os.path.join(input_path, "iter1.jsonl")
    round2_file = os.path.join(input_path, "iter2.jsonl")
    round3_file = os.path.join(input_path, "iter3.jsonl")
    ...
    for round_name in ["round1", "round2", "round3"]:
        ...
        question_to_orig_index = {item["question"]: i for i, item in enumerate(original_items)}
        sorted_results = sorted(round_results[round_name], 
                              key=lambda x: question_to_orig_index.get(x["question"], float('inf')))
        
        with open(scored_file, 'w', encoding='utf-8') as f:
            for scored_result in sorted_results:
                question = scored_result["question"]
                orig_item = next((item for item in original_items if item["question"] == question), None)
                if orig_item:
                    scored_item = {
    ...
    for i in [1, 2, 3]:
        ...
        num_results = len(round_results[round_name])
        if num_results > 0:
            pass_rate = sum(1 for r in round_results[round_name] if is_correct_judgement(r["judgement"])) / num_results * 100
            round_performance[f"Round{i}_Pass@1"] = round(pass_rate, 2)
        else:
            round_performance[f"Round{i}_Pass@1"] = 0.0
    ...
    # NEW: Print token consumption stats
    print(f"Avg. Total Token Consumption: {aggr_statistics['avg_total_token_consumption']:.0f}")
    print(f"Avg. Total Token Consumption (Correctly Solved): {enhanced_statistics['avg_total_token_consumption_correctly_solved']:.0f}")
